...
public class ActiveLogsServiceImpl implements ActiveLogsService {
    ...
    @Override
    @Transactional
    public void cancelSignup(String activeId, String userId) {
        QueryWrapper<ActiveLogs> qw = new QueryWrapper<>();
        qw.eq("active_id", activeId);
        qw.eq("user_id", userId);

        ActiveLogs activeLog = activeLogsDao.selectOne(qw);
        if (activeLog == null) {
            throw new IllegalArgumentException("未找到报名记录");
        }

        activeLogsDao.deleteById(activeLog.getId());

        Activities activity = activitiesDao.selectById(activeId);
